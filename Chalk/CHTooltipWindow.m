...
@implementation CHTooltipWindow
...
+(NSColor*) defaultBackgroundColor
{
  ...
  {
    if (!backgroundColor)
      [CHTooltipWindow setDefaultBackgroundColor:[NSColor colorWithDeviceRed:1.0 green:0.96 blue:0.76 alpha:1.0]];
  }
...
{
  id result =
    [CHTooltipWindow tipWithAttributedString:[[[NSAttributedString alloc] initWithString:tip] autorelease] frame:frame display:display];
  return result;
}
...
{
  CHTooltipWindow* window = [[CHTooltipWindow alloc] init]; // blank slate
  @synchronized(self)
  {
    if (!doneInitialSetup)
    {
      [CHTooltipWindow setDefaultDuration:5];
  ...
  if (display)
    [window orderFrontWithDuration:[CHTooltipWindow defaultDuration]]; // this is affectively autoreleasing the window after 'defaultDuration'
...
-(id) init
{
  ...
  [self setOpaque:NO];
  [self setBackgroundColor:[CHTooltipWindow defaultBackgroundColor]];
